...
public class YbHttp {
    ...
    public static <T> Call send(Request request, HttpResponseListener<T> httpResponseListener) {
        if (com.yb.retrofit.http.RequestMethod.GET.equals(request.getRequestMethod())) {
            return com.yb.retrofit.http.HttpHelper.getAsync(request.getApiUlr()
                    , request.getHeaderMap()
                    , request.getParamsMap()
                    , httpResponseListener);
        } else {
            return com.yb.retrofit.http.HttpHelper.postAsync(request.getApiUlr()
                    , request.getHeaderMap()
                    , request.getParamsMap()
                    , httpResponseListener);
        }
